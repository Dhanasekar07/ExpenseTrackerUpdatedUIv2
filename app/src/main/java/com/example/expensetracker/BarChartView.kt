...
class BarChartView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val barW    = (w - sidePad * 2f) / entries.size
        val barGap  = barW * 0.3f
        ...
        entries.forEachIndexed { i, entry ->
            ...
            canvas.drawRoundRect(
                RectF(left, top, right, bottom), 8f, 8f, barPaint
            )

            // Value on top
            if (entry.value > 0) {
                val label = if (entry.value >= 1000)
                    "$currency${(entry.value / 1000).toInt()}k"
                else
