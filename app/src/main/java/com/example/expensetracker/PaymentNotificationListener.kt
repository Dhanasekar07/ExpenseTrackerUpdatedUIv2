...
class PaymentNotificationListener : NotificationListenerService() {
    ...
    companion object {
        ...
        private val SMS_APPS = setOf(
            "com.truecaller.android","com.google.android.apps.messaging",
            "com.android.mms","com.android.messaging","com.samsung.android.messaging",
            "com.oneplus.mms","com.oneplus.message","com.miui.sms","com.coloros.mms",
            "com.vivo.mms","com.realme.mms","com.asus.message","com.nokia.messaging",
            "com.motorola.messaging","com.textra","com.handcent.nextsms",
            "com.moez.QKSMS","com.klinker.android.evolve_sms","com.dice.truemessenger"
        )
        ...
        private val STRONG_KEYWORDS = listOf(
            "debited","credited","credit alert","debit alert","a/c","ac no","acct",
            "avl bal","avbl bal","available bal","upi ref","upi txn","ref no",
            "upi:","neft","imps","rtgs","your account","your a/c","cyber fraud"
        )

        private val SOFT_KEYWORDS = listOf(
            "paid","received","sent","transferred","payment","transaction","purchase",
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification?) {
        ...
        // ─── THRESHOLD CHECK (Settings) ───────────────────────────────────
        val minAmount = AppPreferences.getMinAmount(this)
        val maxAmount = AppPreferences.getMaxAmount(this)

        if (amount < minAmount) {
            // Silent log — below minimum threshold
            Log.d(TAG, "Amount ₹$amount below min ₹$minAmount — silent log")
            ExpenseDbHelper(this).insertExpense(
                friendlySource(pkg, title), amount, pkg, "silent"
            )
            return
        }

        if (maxAmount > 0 && amount > maxAmount) {
            // Silent log — above maximum threshold
            Log.d(TAG, "Amount ₹$amount above max ₹$maxAmount — silent log")
            ExpenseDbHelper(this).insertExpense(
                friendlySource(pkg, title), amount, pkg, "silent"
            )
            return
